...
local function test_loss(testSet) 
	local logProbs = model:forward(testSet.data)
	
	--local classProbabilities = torch.exp(logProbs)
	--local _, classPredictions = torch.max(classProbabilities,2)
	local testLoss = criterion:forward(logProbs, testSet.labels);
	print(testLoss);
	return testLoss;
end  
------------------------------------------------------------------------
...
for epoch = 1, epochs do
  ...
  local scalar = train.data:size(1)/opt.batch_size;
  losses[#losses+1] = epoch_loss/scalar;  
  print("epoch = " .. epoch .. ', full epoch loss = ' .. epoch_loss)
 
  testLosses[#testLosses+1] = test_loss(test);
...
gnuplot.plot({
  ...
  '-'}, 
  {
  torch.range(1, #testLosses),        -- x-coordinates for data to plot
  torch.Tensor(testLosses),           -- y-coordinates
  '-'})
------------------------------------------------------------------------------
-- TESTING THE LEARNED MODEL
------------------------------------------------------------------------------

local function getClassError(set) 
	local logProbs = model:forward(set.data)
	local classProbabilities = torch.exp(logProbs)
	local _, classPredictions = torch.max(classProbabilities,2)
	count=0;
	for i=1, classPredictions:size(1) do
		if classPredictions[i][1]~=set.labels[i] then count=count+1;
		end
	end
	classError = count*100/classPredictions:size(1);
	return classError;
end

testError=getClassError(test);
trainError=getClassError(train);
